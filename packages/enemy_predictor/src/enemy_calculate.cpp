...
void EnemyPredictorNode::updateArmorDetection(std::vector<cv::Point3f> object_points,
    ...
    cv::Mat tvec = (cv::Mat_<double>(3, 1) << 
                    ...
                    det.position.y(), 
                    det.position.z());
    cv::projectPoints(object_points, visualize_.camera_rvec, tvec, 
                      camera_matrix, dist_coeffs, reprojected_points);
    if (reprojected_points.size() >= 4) {
        cv::rectangle(visualize_.armor_img, reprojected_points[0], reprojected_points[2], 
                      cv::Scalar(255, 0, 0), 2);
    }
    std::vector<cv::Point2f>image_points;
    const cv::Point2f& p0 = image_points[0];
    const cv::Point2f& p1 = image_points[1];
    const cv::Point2f& p2 = image_points[2];
    const cv::Point2f& p3 = image_points[3];
    ...
    visualize_.camara_to_odom = getTrans("camera", "odom");
    det.position = visualize_.camara_to_odom * det.position;
...
void EnemyPredictorNode::ToupdateArmors(const std::vector<Detection>& detections, double timestamp) {
    // 先更新missing_frames
    if (detections.empty()) {
        for (auto& tracker : armor_trackers_) {
            if (tracker.is_valid) {
                tracker.missing_frames++;
            }
        }
        return;
    }
    for(size_t i = 0; i < detections.size(); i++){
        ...
        for (auto& tracker : armor_trackers_) {
            ...
            if (tracker.missing_frames > 50) {
                tracker.is_valid = false;
            }
            tracker.is_active = false;
            
            if(tracker.is_valid && tracker.tracker_idx == detections[i].armor_idx){
              
                tracker.update(detections[i].position, detections[i].armor_idx, timestamp, detections[i].yaw);
                //RCLCPP_INFO(this->get_logger(), "update tracker OK");
                //只在create new tracker时assign to enemy or check or every time ???
                //tracker.assigned_enemy_idx = assignToEnemy(tracker, timestamp);
                ...
                Eigen::Vector3d pos_update = Eigen::Vector3d(tracker.ekf.Xe(0), tracker.ekf.Xe(1), tracker.zekf.Xe(0));
        ...
        if(!has_history_tracker){
            ...
            RCLCPP_INFO(this->get_logger(), "create new tracker");
        }
    }
    for (auto& tracker : armor_trackers_) { 
...
int EnemyPredictorNode::assignToEnemy(ArmorTracker& tracker, double timestamp) {
    ...
    // 4. 创建新敌人
        ...
        int new_enemy_idx = enemies_.size();
        enemies_.emplace_back(new_enemy_idx, tracker.armor_class_id, 0.28);
        
        Enemy& new_enemy = enemies_.back();
        
        tracker.assigned_enemy_idx = new_enemy_idx;
        ...
        new_enemy.yaw_round = 0;
        //new_enemy.armor_tracker_ids.push_back(tracker.tracker_idx);
        findBestPhaseForEnemy(new_enemy, tracker);
        RCLCPP_INFO(this->get_logger(),
                   "Created new enemy %s for tracker %s",
                   new_enemy.enemy_idx,
                   tracker.tracker_idx);
        active_enemies_idx.push_back(new_enemy_idx);
        return new_enemy_idx;
    }
}
void EnemyPredictorNode::EnemyManage(double timestamp, Command& cmd, EnemyPredictorNodeParams& params_) {
    ...
    if(active_enemies_idx.size()== 1){
        ...
        getCommand(enemies_[target_enemy_id], cmd, timestamp, params_);
...
void EnemyPredictorNode::calculateEnemyCenterAndRadius(Enemy& enemy, double timestamp) {
    // 收集活跃装甲板
    std::vector<ArmorTracker*> active_armors;
    for (int tracker_id : enemy.armor_tracker_ids) {
        if(armor_trackers_[tracker_id].is_active){
            active_armors.push_back(&armor_trackers_[tracker_id]);
        }
    }
    
    if (active_armors.empty()) return;
    
    // 根据装甲板数量采用不同的计算方法
    if (active_armors.size() == 1) {
        // 单个装甲板：基于相位推测中心
        double phase_angle = active_armors[0]->phase_id * (M_PI / 2.0);
        double center_yaw = active_armors[0]->yaw - phase_angle + M_PI;
        enemy.center = active_armors[0]->position + enemy.radius * 
                      Eigen::Vector3d(std::cos(center_yaw), std::sin(center_yaw), 0);  //认为armor_z == enemy_z ? ? ?
    } 
    else if (active_armors.size() == 2) {
        // 两个装甲板：使用法向量交点法
        ArmorTracker& armor1 = *active_armors[0];
        ArmorTracker& armor2 = *active_armors[1];
        ...
        if (std::abs(det) > 1e-6) {
            ...
            if (t(0) > 0 && t(1) > 0) {
                ...
                // 交点不在正向，使用几何中心
                useGeometricCenterSimple(enemy, active_armors);
            }
        } else {
            // 行列式为0，方向平行，使用几何中心
            useGeometricCenterSimple(enemy, active_armors);
        ...
        // 多个装甲板：几何中心(no possiblity)
        useGeometricCenterSimple(enemy, active_armors);
    }
 
    enemy.enemy_ckf.update(enemy.center, enemy.yaw, timestamp, active_armors[0]->phase_id);
...
void EnemyPredictorNode::findBestPhaseForEnemy(Enemy& enemy, ArmorTracker& tracker) {
    ...
    for (int idx : enemy.armor_tracker_ids) {
        RCLCPP_INFO(this->get_logger(), "Already has tracker");
        if (idx < 0 || idx >= armor_trackers_.size()) {
            RCLCPP_WARN(this->get_logger(), 
                       "Invalid tracker index %d, armor_trackers_ size=%zu", 
                       idx, armor_trackers_.size());
            continue; 
        }
        if (armor_trackers_[idx].is_valid){
    ...
    if(used_phases.find(best_phase) == used_phases.end()){
        ...
        for(int tracker_idx_ : enemy.armor_tracker_ids){
            if(armor_trackers_[tracker_idx_].is_valid && armor_trackers_[tracker_idx_].phase_id == best_phase){
                if(armor_trackers_[tracker_idx_].phase_conf < tracker.phase_conf){
                   tracker.phase_id = best_phase;
                   // 冲突的tracker, 若conf不如新armor, phase_id设置为-1
                   armor_trackers_[tracker_idx_].phase_id = -1;
                }else{
...
int EnemyPredictorNode::ChooseMode(Enemy &enemy, double timestamp, Command& cmd){
...
void EnemyPredictorNode::getCommand(Enemy& enemy, Command& cmd, double timestamp, EnemyPredictorNodeParams& params_ ){
    cmd.cmd_mode = ChooseMode(enemy, timestamp, cmd);
    ...
    for(int armor_idx : enemy.armor_tracker_ids){
        if(armor_trackers_[armor_idx].is_active){
            active_trackers.push_back(&armor_trackers_[armor_idx]);
        }
    }
    if(cmd.cmd_mode == 1){
        ...
        for(int i = 0; i < active_trackers.size(); i++){
           auto [ball_res, p] = calc_ballistic_((params_.response_delay + params_.shoot_delay), cmd, timestamp, *active_trackers[i], predict_func_double);
    ...
    //visualizeAimCenter(cmd.aim_center);
}
std::pair<Ballistic::BallisticResult, Eigen::Vector3d> EnemyPredictorNode::calc_ballistic_
            (double delay, Command& cmd, double timestamp, ArmorTracker& tracker, std::function<Eigen::Vector3d(ArmorTracker&, double)> _predict_func)
{
    RCLCPP_INFO(this->get_logger(), "Calculating ballistic...");
    
    Ballistic::BallisticResult ball_res;
    Eigen::Vector3d predict_pos_odom;
    Eigen::Isometry3d odom2gimbal_transform = getTrans("odom", "gimbal");
    ...
    RCLCPP_INFO(this->get_logger(), "tick time: %.6f", tick_seconds);
    ...
    RCLCPP_INFO(this->get_logger(), "  ball_res ---------------");
    if (ball_res.fail) {
...
void EnemyPredictorNode::create_new_tracker(const Detection &detection,double timestamp) {
    ...
    armor_trackers_.emplace_back(detection.armor_idx, detection.armor_idx, detection.position, timestamp, detection.yaw, detection.area_2d);
    auto& new_tracker = armor_trackers_.back();
    new_tracker.is_active = true;
    new_tracker.is_valid = true;
    ...
    RCLCPP_INFO(this->get_logger(),
               "Created new tracker %s at (%.3f, %.3f, %.3f)",
               new_tracker.tracker_idx,
...
void EnemyPredictorNode::UpdateCameraData(const std::vector<double> k, const std::vector<double> d){
    visualize_.camera_matrix = (cv::Mat_<double>(3,3) << 
                                k[0], k[1], k[2],
                                k[3], k[4], k[5], 
                                k[6], k[7], k[8]);
    visualize_.dist_coeffs = cv::Mat(frame_info.d); 
    visualize_.camera_heart = cv::Point2f(frame_info.k[2], frame_info.k[5]);
    //cv::circle(visualize_.armor_img, cv::Point(frame_info.k[2], frame_info.k[5]), 8, cv::Scalar(150, 0, 150), -1);
}
